...
# The shape operator is accurate only for regular meshes ! (see Grispun 2006)
...
def regular_triangle_mesh(nx, ny):
    ...
    Vert1 = np.tile(np.arange(0, nx - 1), ny - 1) + np.repeat(
        np.arange(0, nx * (ny - 1), nx), nx - 1
    )
    Vert3 = np.tile(np.arange(0, nx - 1), ny - 1) + np.repeat(
        np.arange(0, nx * (ny - 1), nx), nx - 1
    ) + nx
    Vert2 = Vert3 + 1
    Vert4 = Vert1 + 1
    ...
    Verttmp = np.meshgrid(
        np.arange(0, nx, dtype="float"), np.arange(0, ny, dtype="float")
    )
